      ...
      <div class="w-[70vw] my-5 shadow-md h-[calc(100vh-500px)] rounded-xl overflow-hidden">
         <Grid v-if="aProdutos" :aCabecalhos="['Código', 'Produto', 'Quantidade', 'Valor', 'Desconto', 'Valor total', 'Quantidade Devolução']" :bDataTable="false">
            <tr v-for="(oProduto, iIndice) of aProdutos" :key="iIndice">
               ...
               <td class="p-2"><Campo class="text-right" v-model="aDevolucoes[iIndice].iQuantidadeDevolvida" sTipo="number" :max="oProduto.iQuantidade" min="0" :bObrigatorio="false" @change="validarDevolucao(iIndice, oProduto.iQuantidade)"/></td>
            </tr>
         </Grid>         
      </div>
      <div class="my-5 gap-5 flex mr-auto">
         <Botao sCor="botaoVerde"    sTexto="Confirmar" sLargura="auto" @click="confirmarDevolucao"   :sStyle="{ padding: '0.5rem 1rem' }"/>
...
const props       = defineProps(['aProdutos']);
...
function confirmarDevolucao() {
  const aDevolucoesFiltradas = aDevolucoes.value.filter(item => item.iQuantidadeDevolvida > 0);
   ...
   emit('confirmarDevolucao', aDevolucoesFiltradas);
}
