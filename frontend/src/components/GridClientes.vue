    ...
    <Grid v-if="aClientes" class="mt-10 text-left" :aCabecalhos="['Cliente', 'Nome do Cliente', 'CPF', 'Data de Nascimento', 'Telefone', 'Endereço', 'Ações']" sLayout="0.5fr 1fr 0.6fr 1fr 0.6fr 1.8fr 0.6fr">
        <tr v-for="(oCliente, iIndice) of aClientes" :key="iIndice">
            ...
            <td class="p-2 flex gap-2">
                <span class="cursor-pointer" @click="$emit('showModalCadastro', 3, oCliente)"><i class="fa fa-search p-2 bg-blue-500 rounded-sm text-white"></i></span>
                <span class="cursor-pointer" @click="$emit('showModalCadastro', 2, oCliente)"><i class="fa fa-pencil p-2 bg-yellow-500 rounded-sm text-white"></i></span>
                <span class="cursor-pointer" @click="$emit('showModalExclusao', oCliente.clicodigo)"><i class="fa fa-trash p-2 bg-red-500 rounded-sm text-white"></i></span>
            </td>
        </tr>
    </Grid>
</template>
